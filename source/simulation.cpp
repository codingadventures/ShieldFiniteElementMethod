...
void FEMMesh::init(SimulationParameters* params)
{
	...
	plane.damping = 0;
	LOGI( "Plane d = %f stiffness = %f damping = %f", plane.d, plane.stiffness, plane.damping );
	...
	if (params->fixedHeight > 0)
	{
		...
		LOGI( "Fixed box = %f    %f", bbox[0] ,  TCoord(bbox[1][0],maxY,maxZ) );
	...
	for (int eindex = 0; eindex < nbe; ++eindex)
	{
		...
		if (vol36<0)
		{
			LOGI("ERROR: Negative volume for tetra ",eindex," <",A,',',B,',',C,',',D,"> = ",vol36/36);
...
#endif
		LOGI("FEM init done. Particles: %d Tetrahedra: %d ", positions.size()  , tetrahedra.size());
...
bool FEMMesh::save(const std::string& filename)
{
	...
	if (!out)
	{
		LOGI("Cannot write to file %s" , filename.c_str() );
...
bool FEMMesh::load(const std::string& filename)
{
	...
	if (!in)
	{
		LOGI("Cannot open file  %s" , filename.c_str()); 
		return false;
	}
	int nbp = 0, nbc = 0;
	in >> nbp >> nbc;
	...
	{
		LOGE( "ERROR: file %s contains %d vertices while the mesh contains %d" , filename.c_str() ,  nbp ,  positions.size());

		return false;
	}
	if (nbc != 6)
	{
		LOGE( "ERROR: file %s contains %d values instead of 6" , filename.c_str()  , nbc ); 
		return false;
	}
	for (unsigned int i=0;i<positions.size();++i)
	{
...
void showDebug(const TVec& v, const char* name)
{
	...
	{
		for (unsigned int i=0;i<5;++i) ostr << ' ' << v[i];
		ostr << " ...";
		for (unsigned int i=v.size()-5;i<v.size();++i) ostr << ' ' << v[i];
	}
	str <<  ostr.rdbuf();
	output = str.str();
	LOGI("showDebug: %s ",output.c_str());
}
...
void linearSolver_ConjugateGradient(const SimulationParameters* params, FEMMesh* mesh, MechanicalMatrix matrix)
{
	...
	while (i < maxIter && delta_new > delta_threshold)
	{
		...
		double den = dot_dq;
		if (verbose >= 2) LOGI("CG i= %d den = %f",i , den);
		double alpha = delta_new / den;
		if (verbose >= 2) LOGI("CG i= %d  alpha = %f",i , alpha );
		...
		if (verbose >= 2) LOGI("CG i= %d delta = %f",i, delta_new );
	...
	if (verbose >= 1) 
		LOGI("CG iterations = %d residual error = %f", i, sqrt(delta_new / delta_0) );
}
