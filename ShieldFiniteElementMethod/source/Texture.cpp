...
bool Texture::Load(std::string directory)
{
	...
	if ((file = fopen(m_file_name.c_str(), "rb")) == NULL)
	{
		LOGE("File not found : %s", m_file_name.c_str());
		return 0;
	}
	...
	if (PNG_reader == NULL)
	{
		LOGE("png_create_read_struct failed for file %s", m_file_name.c_str());
		fclose(file);
		return 0;
	}
	...
	{
		LOGE( "png_create_info_struct failed for file %s", m_file_name.c_str());
	...
	{
		LOGE("Loading failed for PNG file %s", m_file_name.c_str());
	...
	LOGI("PNG image %s: %dx%dx%d", m_file_name.c_str(),width,height,bit_depth*channels);
	bool changed = false;
	...
	*/
	if (changed)
	{
		...
		channels = png_get_channels(PNG_reader, PNG_info);
		color_type = png_get_color_type(PNG_reader, PNG_info);
		LOGI("Converted PNG image %s: %dx%dx%d", m_file_name.c_str(),width,height,bit_depth*channels);
	}
	...
	case 16:
		...
		break;
	default:
		LOGE("PNG: in %s, unsupported bit depth: %d",m_file_name.c_str(), bit_depth);
		return 0;
	}
