...
void main (void)
{
    ...
    vec4 base = texture2D(colorMap, tex_coord);
    //gl_FragColor = base;    return;
    
    vec3 bump = normalize( texture2D(normalMap, tex_coord).xyz - 0.5 );
   ...
   // vec4 vSpecular = vec4((specular1+specular2) * base.a);
    ...
    gl_FragColor = base //( (vAmbient +vDiffuse )*(base *(1.2+0.3*base.a) ) + 
                    // vSpecular
                   //)
				   ;
}
